...
#Le niveau est fini
	if toutfini == 1:
		bsuivantimg = pygame.image.load("sprite/refaire.png").convert_alpha()
		bretcustom = bsuivantimg.get_rect() #Pour que Rect représente avec précision l'image chargé
		bretcustom = bretcustom.move(600, 340)
			...
			for event in pygame.event.get():
				if event.type == MOUSEBUTTONUP:
					if bretmenu.collidepoint(event.pos):
						return 1, False
					if bretcustom.collidepoint(event.pos):
						return 3, False
			...
			for event in pygame.event.get():
				...
				if event.type == pygame.KEYDOWN and toutfini == 0:
					if event.key == pygame.K_ESCAPE:
						return 2, True
def jeu(salle, perso1, perso2, perso3, perso4):
	continuer = True
	debug = 1
